...
def fechar_pedido(request):
    exames_id = request.POST.getlist('exames')
    solcitacao_exames = TiposExames.objects.filter(id__in=exames_id)
    ...
    for exame in solcitacao_exames:
...
def cancelar_pedido(request, pedido_id):
    ...
    if not pedido.usuario == request.user:
        messages.add_message(request, constants.ERROR, 'Pedido não poder ser cancelado. Pedido não é do usuário' )
        return redirect('/exame/gerenciar_pedidos/')
...
def solicitar_senha_exame(request, exame_id):
    ...
    if request.method == "GET":
        ...
        if senha == exame.senha:
            ...
            return redirect(f'/exame/solicitar_senha_exame/{exame.id}')
...
def gerar_acesso_medico(request):
    if request.method == "GET":
        ...
        return redirect('/exame/gerar_acesso_medico')
